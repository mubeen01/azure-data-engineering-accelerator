...
-- etl.usp_load_dim_customer_retail — same reconciliation as
-- examples/banking/sql/03_Load_Dim_Customer_Banking.sql and
-- examples/healthcare/sql/05_Load_Dim_Patient_Healthcare.sql: the generator's
-- customer records embed a single address directly, so location_id is
-- synthesized as location_id = customer_id before the SCD Type 2
-- expire-then-insert. Writes into the *generic* dim.dim_customer
-- (src/sql/03-dimensions/05_Create_Dim_Customer.sql) — retail customers
-- and banking customers coexist in the same table, distinguished by
-- source_system ('RETAIL_CORE' vs 'BANKING_CORE'), exactly as
-- src/sql/00-standards/naming-standards.md's schema design intends. No new
-- dimension table needed for retail customers, unlike banking's
-- dim_account/dim_loan or healthcare's dim_patient/dim_provider.
-- =============================================================================

USE [AdeaDW];
GO

CREATE OR ALTER PROCEDURE etl.usp_load_dim_customer_retail
AS
BEGIN
    SET NOCOUNT ON;
    ...
    VALUES ('etl.usp_load_dim_customer_retail', SYSUTCDATETIME(), 'RUNNING');
    SET @v_run_key = SCOPE_IDENTITY();

    BEGIN TRY
        BEGIN TRANSACTION;

        -- Step 1: upsert dim_location (SCD1), same as the banking/healthcare
        -- customer loaders.
        ...
        USING (
            SELECT DISTINCT
                ...
                stg.postal_code,
                stg.country
            FROM staging.stg_retail_customer AS stg
        ...
        -- Step 2: SCD Type 2 load of dim_customer.
        ;WITH src AS (
            SELECT DISTINCT
                stg.source_system,
                stg.customer_id,
